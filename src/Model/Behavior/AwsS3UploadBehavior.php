<?php
namespace PowerSystem\S3UploadSDK\Model\Behavior;
...
use PowerSystem\S3UploadSDK\File\Writer\AwsS3Writer;
...
class AwsS3UploadBehavior extends Behavior
{
	...
	protected function getS3Config(array $config)
	{
		...
		if(!$s3_key) 	throw new Exception(__d('PowerSystem/S3UploadSDK', 'AwsS3.key setting missing'));
		if(!$s3_secret) throw new Exception(__d('PowerSystem/S3UploadSDK', 'AwsS3.secret setting missing'));
		if(!$s3_region) throw new Exception(__d('PowerSystem/S3UploadSDK', 'AwsS3.region setting missing'));
		if(!$s3_bucket) throw new Exception(__d('PowerSystem/S3UploadSDK', 'AwsS3.bucket setting missing'));

		if(!Configure::check('AwsS3.base_url')){
			throw new Exception(__d('PowerSystem/S3UploadSDK', 'AwsS3.base_url setting missing'));
		}
		...
		foreach ($this->getConfig(null, []) as $field => $config) {
			...
			if($images_only){
				$validator->add($field, 'mimeType', [
	                ...
	                'message' => __d('PowerSystem/S3UploadSDK', 'Invalid file type. Please upload images only (gif, png, jpg).'),
	                'last' => true
	            ]);
			}
			...
			$validator
	            ...
	            ->add($field, 'fileUnderPhpSizeLimit', [
	                ...
	                'message' => __d('PowerSystem/S3UploadSDK', 'This file is too large'),
	                'provider' => 'upload'
	            ])
	            ...
	            ->add($field, 'fileCompletedUpload', [
	                ...
	                'message' => __d('PowerSystem/S3UploadSDK', 'This file could not be uploaded completely'),
	                'provider' => 'upload'
	            ])
	            ->add($field, 'fileFileUpload', [
	                'rule' => 'isFileUpload',
	                'message' => __d('PowerSystem/S3UploadSDK', 'There was no file found to upload'),
	                'provider' => 'upload'
	            ])
	            ->add($field, 'fileSuccessfulWrite', [
	                'rule' => 'isSuccessfulWrite',
	                'message' => __d('PowerSystem/S3UploadSDK', 'This upload failed'),
	                'provider' => 'upload'
	            ]);
		}
